...
- (MGLRow)preferredRow {
    ...
    for (NSUInteger i = 0; i < count; i++) {
        CLLocationDistance distance = useMetric ? MGLMetricTable[i].row.distance : MGLImperialTable[i].row.distance;
        if (distance <= maximumDistance) {
            row = useMetric ? MGLMetricTable[i].row : MGLImperialTable[i].row;
        } else {
            break;
        }
    }
